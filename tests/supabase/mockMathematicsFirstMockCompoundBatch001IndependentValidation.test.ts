 ...
 * Independent Validation Promotion (Decision 165). Structural tests
 * against migration 111's own SQL text, mirroring tests/supabase/
 * mockMathematicsBatch003IndependentValidation.test.ts's own established
 * convention (migration 101, Decision 158) exactly.
 */

const sql = fs.readFileSync("supabase/migrations/111_mock_mathematics_firstmock_compound_batch001_independent_validation.sql", "utf8");
...
const EXPECTED_1_FAMILY = ["mock-mr03mr07-perimeterarea"];

test("exact 4-question allow-list: v_target_ids contains exactly the 4 migration-109 IDs, no more no less", () => {
...
test("exact one-family membership: v_target_families contains exactly the 1 approved family, no more no less", () => {
  ...
  assert.deepEqual(families, EXPECTED_1_FAMILY);
});
...
test("the 4 grouping columns are never referenced anywhere in this migration's real SQL -- explicit immutability, and explicit separation from Decision 164's own distinct, unapplied structural-normalisation proposal", () => {
...
test("zero overlap with any Batch 001/002/003 or existing costumeschedule target ID -- no prior batch can be accidentally re-touched by this migration", () => {
  const PRIOR_IDS = [
