 ...
 * Review (Decision 205/206). Structural tests against migration 141's own
 * SQL text, mirroring migration 138's own established pending-review
 * placeholder pattern.
 */

const sql = fs.readFileSync("supabase/migrations/141_mock_mathematics_structural_capacity_increment006_pending_review.sql", "utf8");
const executable = sql.split("\n").filter((line) => !line.trimStart().startsWith("--")).join("\n");

test("registers exactly one family, mock-mr11-roundingbounds, as pending_independent_review", () => {
  const inserts = [...executable.matchAll(/insert into public\.ali_family_review/g)];
  assert.equal(inserts.length, 1);
  assert.match(executable, /'question_family', 'mock-mr11-roundingbounds', 'UNASSIGNED'/);
  assert.equal((executable.match(/'pending_independent_review'::public\.family_review_decision/g) || []).length, 1);
});

test("review_type is mock_maths_independent_review, using the new INCREMENT006 marker (distinct from INC001/WAVE002/CORRECTION001/INCREMENT003/INCREMENT004/INCREMENT005)", () => {
  ...
  assert.match(executable, /MOCK-STRUCTURAL-CAPACITY-INCREMENT006/);
  ...
  assert.ok(!executable.includes("MOCK-STRUCTURAL-CAPACITY-INCREMENT005"));
});
...
test("marker does not collide as a substring with any prior marker, and no prior marker is a substring of it", () => {
  ...
  const priorMarkers = [
    ...
    "MOCK-STRUCTURAL-CAPACITY-INCREMENT004", "MOCK-STRUCTURAL-CAPACITY-INCREMENT005",
  ];
...
test("notes carry the exact batch marker and all four question IDs", () => {
  assert.match(executable, /MOCK-STRUCTURAL-CAPACITY-INCREMENT006 new content review: mock-mr11-roundingbounds/);
  assert.match(executable, /mock-mr11-roundingbounds-01, mock-mr11-roundingbounds-02, mock-mr11-roundingbounds-03, mock-mr11-roundingbounds-04/);
});
...
test("idempotency guard: where not exists checks family_id + decision + review_type + notes together", () => {
  ...
  assert.match(executable, /where family_id = 'mock-mr11-roundingbounds' and decision = 'pending_independent_review'/);
});
...
test("wrapped in a single begin/commit transaction, header discloses NOT APPLIED and dependency on migration 140", () => {
  assert.equal((executable.match(/\bbegin;/g) || []).length, 1);
  assert.equal((executable.match(/\bcommit;/g) || []).length, 1);
  assert.match(sql, /NOT APPLIED\. Founder must apply/);
  assert.match(sql, /migration 140/);
});
