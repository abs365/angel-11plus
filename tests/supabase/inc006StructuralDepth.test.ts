...
/**
 * Stage 3, Increment 006 — structural tests against migration 081's own
 * SQL text, matching the established convention
 * (tests/supabase/mr04ContentDepthFoundation.test.ts): string/regex
 * assertions against the raw migration file, never a live database.
 * This migration is drafted for Founder/reviewer application and has
 * not been applied.
 */

const sql = fs.readFileSync("supabase/migrations/081_inc006_structural_depth.sql", "utf8");
...
test("NEW CONTENT BECOMES ELIGIBLE ONLY THROUGH THE APPROVED STATE: every inserted row is 'provisional' -- never 'practice_eligible' or 'mock_eligible'", () => {
  ...
  assert.equal(eligibilityValues.length, 8, "expected exactly 8 rows");
  for (const v of eligibilityValues) assert.equal(v, "provisional");
});

test("exactly 8 INSERT statements, one per new question, each idempotent via ON CONFLICT DO NOTHING", () => {
...
test("every row targets one of the two approved skills, and only those two", () => {
  const skills = [...executable.matchAll(/'maths', '(QT-MR-\d+)'/g)].map((m) => m[1]);
  assert.equal(skills.length, 8);
  for (const s of skills) assert.ok(["QT-MR-12", "QT-MR-08"].includes(s));
});

test("QT-MR-02 and QT-RC-07 are never mentioned -- explicitly out of scope for this increment", () => {
  assert.ok(!executable.includes("QT-MR-02"));
  assert.ok(!executable.includes("QT-RC-07"));
});
...
test("every row is content_difficulty='hard' -- the confirmed-missing tier, never a different value", () => {
  ...
  assert.equal(difficulties.length, 8);
  for (const d of difficulties) assert.equal(d, "hard");
});

test("family_id matches one of the two declared new families for every row, never null and never an existing family", () => {
  const familyIds = [...executable.matchAll(/\n\s*'(mr0[13]-[a-z-]+)', 'angel_original'/g)].map((m) => m[1]);
  assert.equal(familyIds.length, 8);
  const allowed = new Set(["mr01-reverse-mean", "mr03-coord-combined"]);
