 ...
 * Review (Decision 195/196). Structural tests against migration 135's
 * own SQL text, mirroring migration 132's own established pending-review
 * placeholder pattern.
 */

const sql = fs.readFileSync("supabase/migrations/135_mock_mathematics_structural_capacity_increment004_pending_review.sql", "utf8");
const executable = sql.split("\n").filter((line) => !line.trimStart().startsWith("--")).join("\n");

test("registers exactly one family, mock-mr04-campingsale, as pending_independent_review", () => {
  const inserts = [...executable.matchAll(/insert into public\.ali_family_review/g)];
  assert.equal(inserts.length, 1);
  assert.match(executable, /'question_family', 'mock-mr04-campingsale', 'UNASSIGNED'/);
  assert.equal((executable.match(/'pending_independent_review'::public\.family_review_decision/g) || []).length, 1);
});

test("review_type is mock_maths_independent_review, using the new INCREMENT004 marker (distinct from INC001/WAVE002/CORRECTION001/INCREMENT003)", () => {
  ...
  assert.match(executable, /MOCK-STRUCTURAL-CAPACITY-INCREMENT004/);
...
test("notes carry the exact batch marker and all four question IDs", () => {
  assert.match(executable, /MOCK-STRUCTURAL-CAPACITY-INCREMENT004 new content review: mock-mr04-campingsale/);
  assert.match(executable, /mock-mr04-campingsale-01, mock-mr04-campingsale-02, mock-mr04-campingsale-03, mock-mr04-campingsale-04/);
});
...
test("idempotency guard: where not exists checks family_id + decision + review_type + notes together", () => {
  ...
  assert.match(executable, /where family_id = 'mock-mr04-campingsale' and decision = 'pending_independent_review'/);
});
...
test("wrapped in a single begin/commit transaction, header discloses NOT APPLIED and dependency on migration 134", () => {
  assert.equal((executable.match(/\bbegin;/g) || []).length, 1);
  assert.equal((executable.match(/\bcommit;/g) || []).length, 1);
  assert.match(sql, /NOT APPLIED\. Founder must apply/);
  assert.match(sql, /migration 134/);
});
