...
**Consolidation is required, not optional**: several different database family records (different authoring wave, different naming convention, even a lone `row_count=1` record) can represent the SAME genuine reasoning demand and must be merged into one educational family — never counted twice, and never kept separate merely because their `family_id` values differ. Conversely, two families must be kept separate whenever the actual learner demand differs materially (e.g. explaining a word's meaning vs. matching a synonym; a plain multi-select vs. a select-then-justify task), even if both are informally "about the same topic." `lib/ali/familyTaxonomy.ts`'s `ENGLISH_EDUCATIONAL_FAMILY_CONSOLIDATION` (13 genuine educational families, consolidated from 18 named database family records covering 140 rows — the remaining 62 of English's 80 database family records are assessment/passage/question-type-scoped groupings, not yet confirmed or denied as additional educational families) and `WRITING_EDUCATIONAL_FAMILY_MODEL` (Writing's entire cited 16-family estate consolidates to exactly ONE genuine educational family, "Reflective/Discursive Writing Response" — topic is a prompt/task variant dimension, never a separate family) are the durable, tested record of this consolidation as of 2026-09-06. Both are disclosed `heuristic` confidence throughout — built from family names and row counts, not a live per-question content review — and must be corrected, not silently trusted forever, once real content review becomes possible.

**94 English passage-bound groups (`lib/ali/englishFamilyModel.ts`) are not a contradiction of 80 database family records or 13 genuine educational families** — they are a different lens entirely: a passage-bound group keys on (passage, reasoning-pattern), so the SAME educational family (e.g. Retrieval) produces a different passage-bound group for every distinct passage it appears in. More passage-bound groups than database families is expected, not anomalous, and neither number is itself "genuine educational depth" — only the consolidated educational-family count is.
...
- **2026-09-06 (Migration 232 Production Reconciliation)** — Section 24 (Content Retirement/Versioning). The Educational Foundation Completion increment's own root-cause diagnosis for the Writing 17-rows/16-families discrepancy was DISPROVEN by direct post-application production evidence. It had inferred that migration 228's stale `row_count` was hiding a second live row inside an existing Writing family. Post-application evidence (`writing_total_rows=17`, `writing_rows_with_family=16`, `writing_rows_without_family=1`) instead proves the cause was ONE Writing bank row with `family_id IS NULL` (identified as `wrt-003`) — simple arithmetic, not staleness. This does not invalidate Section 24's own standing rule (`ali_question_family`'s aggregate fields must be kept live by a trigger, not treated as a snapshot) — that architectural weakness was real and independently confirmed fixed (`stale_family_row_counts = 0` post-application). Full record: `ANGEL_EDUCATIONAL_FOUNDATION_COMPLETION_REPORT.md` Section B, and migration 232's own CORRECTION HISTORY.
- **2026-09-06 (Final Educational Family Classification & Foundation Acceptance Gate)** — Section 2 (Educational Family Definition). Closes the Foundation's last open blocker: English's 80 and Writing's 16 database family records are now consolidated into 13 and 1 genuine educational families respectively (`ENGLISH_EDUCATIONAL_FAMILY_CONSOLIDATION`/`WRITING_EDUCATIONAL_FAMILY_MODEL`, `lib/ali/familyTaxonomy.ts`), both heuristic confidence, both tested. `wrt-003` (the one unfamilied Writing row, `subject='writing'`, `skill=QT-WC-01a`, `eligibility_status='provisional'`) is dispositioned REMAIN PROVISIONAL/UNCLASSIFIED — it shares the same competency as the single Writing educational family in substance, but its specific prompt/topic content is not available to this session to confirm or rule out duplication with an existing prompt variant, and its `provisional` status already correctly withholds it from any learner exposure. No production data was mutated. Full record: `ANGEL_EDUCATIONAL_FOUNDATION_ACCEPTANCE_REPORT.md`.
