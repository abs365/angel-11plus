...
// The 4 (of 40) published Wave 2 candidates whose own correct answer is
// genuinely short (a letter-code sequence or a single word) and is
// therefore structurally unscorable by the LEGACY_HEURISTIC path
// Migration 242 routed all 40 through -- confirmed deterministically,
// against the real, imported scoreEnglishComprehensionAnswer/
// scoreEnglishAnswer functions, immediately before writing this file:
// typing the candidate's own exact correct answer scores 0, because
// scoreEnglishAnswer() unconditionally rejects any answer under 8
// characters ("b, a, c" / "c, a, b" are 7 characters; "nervous" is 7),
// and because extractKeywords() only keeps words longer than 3
// characters, so a letter-code answer like "b, d, c, a" (10 characters,
// clears the length floor) still yields zero extractable keywords and
// a 0/0 ratio. This is a pre-existing, deliberate anti-gaming floor in
// scoreEnglishAnswer (practiceContent.ts) -- not touched by this
// migration, and not something this repair second-guesses.
const TARGET_IDS = [
  "ei003-w2-seq-explicit-01",
  "ei003-w2-seq-dispersed-02",
  "ei003-w2-seq-narrative-order-02",
  "ei003-w2-emotion-action-01",
];
...
const idListSql = TARGET_IDS.map((id) => sqlString(`qf-${id}`)).join(", ");

const preconditionChecks = TARGET_IDS.map(
  (id) => `  if exists (
    select 1 from public.ali_question_bank
    where id = ${sqlString(`qf-${id}`)}
      and prompt ? 'validationTier'
  ) then
    raise exception 'Migration 243 precondition failed: qf-${id} already has a validationTier field in prompt -- refusing to proceed to avoid overwriting a value this migration did not originate. Investigate before re-running.';
  end if;`
).join("\n");

const updateStatements = TARGET_IDS.map(
  (id) => `update public.ali_question_bank
set prompt = prompt || jsonb_build_object('validationTier', 'TIER2_ACCEPTED_SET')
where id = ${sqlString(`qf-${id}`)};`
).join("\n\n");

const postconditionChecks = TARGET_IDS.map(
  (id) => `  if not exists (
    select 1 from public.ali_question_bank
    where id = ${sqlString(`qf-${id}`)}
    ...
    raise exception 'Migration 243 postcondition failed: qf-${id} does not have the expected validationTier after update, or lost a pre-existing field.';
  end if;`
).join("\n");

const sql = `-- Angel Digital 11+ — Migration 243
...
fs.writeFileSync(new URL("../supabase/migrations/243_ei003_wave2_short_answer_tier_repair.sql", import.meta.url), sql);
console.log("Wrote supabase/migrations/243_ei003_wave2_short_answer_tier_repair.sql");
console.log("Repaired candidate count:", TARGET_IDS.length);
