/**
 * Mathematics First Mock — shared candidate-pool reconstruction (Decision
 * 210 Part 6, Decision 212, factored out in Decision 214 to be shared
 * between `scripts/mock-mathematics-composition-report.mjs` and
 * `scripts/mock-mathematics-first-mock-curation.mjs` rather than
 * duplicated). Reconstructs the real, current 77-row Mathematics
 * `mock_eligible` estate DIRECTLY FROM MIGRATION SOURCE (no live database
 * access in this environment) -- every row transcribed from its own
 * source migration, cited inline, never assumed from Decision-log prose.
 * `marks` is `1` on EVERY row: the Mathematics Marking Integrity Gate
 * (migrations 117/118) corrected every row's own `prompt.marks` from its
 * originally-authored value (1 or 2) down to a uniform 1 -- this module
 * applies the corrected value, not the as-authored one.
 *
 * Real question/answer/sharedStem/stimulus text is sourced from
 * `mock-mathematics-source-content.json`, extracted verbatim (a one-off,
 * disclosed regex pass over each source migration's own
 * `$json$...$json$` prompt literal, never hand-retyped) from migrations
 * 088/091/095/113/119/125/131/134/137/140.
 */
...
/** Real question/answer/sharedStem/stimulus for `id` if extracted, else an honest placeholder (never fabricated) for any row this module's own extraction pass did not cover. */
...
/**
 * Builds the full 77-row Mathematics candidate pool (73 `mock_eligible` +
 * 4 `independently_validated` Perimeter Area rows) and self-checks it
 * before returning. Throws if the reconstruction does not match the
 * established Decision-210-through-211 baseline (77 mock_eligible
 * rows/marks/33 experiences) -- callers must never trust this pool
 * silently.
 */
export function buildMathematicsPool() {
  const pool = [];
  ...
  // rows/marks/33 experiences before this module trusts its own data.
  // =========================================================================
  ...
  if (perimeterAreaRows.length !== 4) {
    throw new Error(`Reconstruction self-check FAILED: expected 4 independently_validated (Perimeter Area) rows, found ${perimeterAreaRows.length}.`);
  }
