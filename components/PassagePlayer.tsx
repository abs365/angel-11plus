  ...
  return (
    <div className="mb-4 rounded-xl border border-gray-100 bg-gray-50 px-3 py-2.5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {hasTTS && listenState === "idle" && recordState !== "recording" && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-purple-700 bg-purple-50 hover:bg-purple-100 px-3 py-1.5 rounded-lg transition-colors"
          >
            <Volume2 size={13} />
            Listen
          </button>
        ...
        {hasTTS && listenState === "playing" && (
          <>
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold text-purple-700 bg-purple-100 hover:bg-purple-200 px-3 py-1.5 rounded-lg transition-colors"
            >
              <span className="flex gap-0.5">
                <span className="w-1 h-3 bg-purple-600 rounded-sm inline-block" />
                <span className="w-1 h-3 bg-purple-600 rounded-sm inline-block" />
              </span>
              Pause
            </button>
            <button
              ...
              className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5 rounded-lg transition-colors"
            >
              <Square size={10} className="fill-current" />
              Stop
            </button>
            <span className="text-xs text-purple-500 animate-pulse font-medium">Reading…</span>
          </>
        )}
        ...
        {recordState === "recording" && (
          <>
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 px-3 py-1.5 rounded-lg transition-colors"
            >
              <Square size={10} className="fill-current" />
              Done
            </button>
      ...
      {recordState === "recording" && liveTranscript && (
        <p className="mt-2 text-[11px] text-gray-400 italic leading-snug line-clamp-2">
          ...
          <div className="flex items-end gap-5 flex-wrap">
            <Stat
              ...
              color={
                ...
                : result.accuracy >= 60 ? "text-amber-500"
                : "text-red-500"
              }
            />
            <Stat value={String(result.wpm || "—")} label="WPM" />
            <Stat value={`${result.wordsMatched}/${result.wordsTotal}`} label="Words" />
            <p className="text-xs text-gray-500 leading-snug flex-1 min-w-[120px]">
              {result.accuracy >= 90
      ...
      {recordState === "done" && !result && (
        <div className="mt-2 flex items-center gap-2">
          <span className="text-xs text-gray-400">No speech detected.</span>
          <button onClick={resetReading} className="text-xs text-purple-600 hover:underline">
            Try again
          </button>
        </div>
      )}
