...
export function DataTableStimulus({ stimulus }: { stimulus: MockTableStimulus }) {
  ...
  return (
    <div className="my-4 rounded-lg border border-[var(--angel-border)] overflow-x-auto">
      <table className="w-full text-sm border-collapse" aria-describedby={stimulus.caption ? captionId : undefined}>
        <caption
          id={captionId}
          className={stimulus.caption ? "text-left px-3 pt-3 pb-1 text-xs font-semibold text-[var(--angel-muted)]" : "sr-only"}
        >
          {stimulus.caption ?? "Data table"}
        </caption>
        <thead>
          <tr className="border-b border-[var(--angel-border)]">
            {stimulus.headers.map((header, index) => (
              <th
                key={index}
                scope="col"
                className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-[var(--angel-muted)]"
              >
                {header}
              </th>
            ))}
        ...
        <tbody>
          {stimulus.rows.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              className={rowIndex % 2 === 1 ? "bg-[var(--angel-sky)]" : undefined}
            >
              {row.map((cell, cellIndex) => (
                <td key={cellIndex} className="px-3 py-2 text-sm text-[var(--angel-ink)] tabular-nums whitespace-pre-line">
                  {cell}
                </td>
              ))}
            </tr>
