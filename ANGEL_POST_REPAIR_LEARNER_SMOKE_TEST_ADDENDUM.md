...
- **Deployment self-correction disclosed:** the first attempt to commit `candidateStoreMapping.ts`/`englishSynonymFamily.ts` (the forward-path TS fix) turned out to be inseparably entangled with a much larger, still-uncommitted "Controlled Content Manufacturing Wave 1" body of work (a newer `types.ts` shape, a Maths-side `independentValidation.ts`, several `mr0*Blueprints.ts` files) — committing it broke Vercel's production build (TypeScript could not resolve the real dependency graph). This was caught immediately after push, before it reached the production alias (Vercel does not promote a failed build; the prior deployment kept serving throughout, confirmed no outage occurred). Resolved by removing `candidateStoreMapping.ts` and its whole dependency cluster from git (kept on disk, untracked, for whoever picks up that separate Wave-1 arc) — this file is manufacturing-time-only tooling, never imported by any live Next.js route, so its absence from git does not affect the deployed application. The actual production-authoritative fix (migrations 239/240, pure SQL) was never affected by this and remains fully applied and verified. Re-verified via a genuinely isolated `git worktree` checkout (not just the main working directory, which still had the untracked files physically present and so could not by itself prove what a fresh clone would see) before the corrected push.
