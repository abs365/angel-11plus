...
## 25. Competency-Denominator Terminology Reconciliation (Increment 020 addendum)

Additive correction, no prior content deleted or reinterpreted. Increment 020's own reporting used "2 of 6 Mathematics competencies" / "3 of 6" for teaching coverage, which appeared to contradict §10's and §24.10's own "2/12" figure. Traced to source; both numbers are correct, at two different, nameable abstraction levels — the earlier "2/12" phrasing (§10, §24.10) was itself imprecisely labelled ("Mathematics 2/12") when 12 is not a Mathematics-only count. Resolved as follows, so future reports use one named metric per statement rather than repeating that ambiguity:

- **12 = `ALL_COMPETENCY_IDS`** (`lib/learningEngine/assessmentBrainMap.ts:111`) — `Object.keys(COMPETENCIES)` (13 raw keys: `RC-01..04`, `AR-01`, `MR-01..06`, `WC-01/02`) filtered to exclude `AR-01` (Applied Reasoning, excluded from mastery-tracking per CSSE Completion Programme Decision 58). This is the **whole-programme, currently-iterated competency set across all three assessed components at once**: English Comprehension (`RC-01..04`, 4), Mathematics (`MR-01..06`, 6), Continuous Writing (`WC-01/02`, 2) — 4+6+2=12. §10's "2/12" and §24.10's "(2/12, 0/9)" both use this whole-programme denominator; the "0/9" figure for English is a *different, unrelated* denominator again (9 distinct free-text Reading `skill` values, §4, not a competency-ID count at all — English's own teaching-coverage figure was never comparable to Mathematics's in the first place).
- **6 = Mathematics-only competencies** — the same `COMPETENCIES` map filtered to `component === "Mathematics"` (`MR-01` through `MR-06`). Not a separately named export as of this writing (Increment 020 derived it directly from the same source); a plain subset filter of the 12 above, not a different taxonomy or a "raw vs filtered" distinction — RC/WC simply don't belong to Mathematics.

**Both are correct simultaneously and measure different things**: "2/12" (before Increment 020) = 2 of the whole programme's 12 currently-iterated competencies have a full lesson, both of which happen to be Mathematics ones. "2/6" (before) / "3/6" (after Increment 020 Wave 1) = the same 2 (then 3) lessons, expressed against the Mathematics-only subset of 6. Neither number is wrong; the defect was leaving the denominator unnamed. **Authoritative terminology going forward**: state "N of 6 Mathematics (MR-series) competencies" for a Mathematics-scoped claim, and "N of the 12 programme-wide currently-iterated competencies (`ALL_COMPETENCY_IDS`)" for a whole-programme claim — never bare "Mathematics competencies" when the number in play is actually the whole-programme count, or vice versa. No architecture changed to produce this reconciliation; it is a documentation/reporting-terminology fix only.

---

## 26. Increment 020 Production Outcome (additive — no prior figures rewritten)

**Increment 020 = PRODUCTION COMPLETE / LIVE VERIFIED (2026-09-04).** Migrations 222, 223, and 224 all Founder-applied and independently live-verified via read-only production query at each step (never assumed from execution success alone). New family `mr03-compound-area-perimeter` (8 rows, MR-03/QT-MR-07) is LIVE IN PRACTICE: `eligibility_status = practice_eligible` for all 8, zero Mock exposure, zero unexpected rows, full additive review history (content review `approved_with_amendment` + a separate `amendment_verification`/`approved` row, both real, non-`UNASSIGNED` reviewer identities) preserved intact. Visual browser render verification remains DEFERRED — a distinct, still-open item never resolved by production/governance closure.

**Baseline deltas from this closure (Mathematics only; §2's own headline table above is left unedited, per this section's own additive-only discipline):**
- Live Mathematics question count: 293 → **301** (+8).
- Live Mathematics family count: 73 → **74** (+1, `mr03-compound-area-perimeter`) — still the only Practice-track family covering a compound/multi-rectangle shape.
- Learner-reachable (Practice-eligible) Mathematics questions: ~194 (§3's own live anchor) → **~202** (+8, same anchor basis, not independently re-queried this session).
- Full Mathematics teaching-sequence coverage (§25's own now-authoritative Mathematics-only denominator): 2/6 → **3/6** (MR-03 compound-shape lesson added, `/learning-intelligence/learn/mathematics/compound-shapes`).
- Mathematics Mock-track reserve: **unchanged** — this family never touched it (Founder's own explicit correction this increment: reserve remains 21 rows/21 marks, not inflated by migration 182, which stays HOLD/NOT APPLIED).
- Mathematics Visual Education: first diagram of any kind now live (§3's own "zero diagrams anywhere" finding is the FIRST instance this closes, for MR-03 only — the other 5 Mathematics competencies remain TEXT_SUFFICIENT/VISUAL_USEFUL per the Increment 020 Visual Education Matrix, `lib/ali/visualEducationMatrix.ts`, not VISUAL_REQUIRED).

**English Reading and Continuous Writing: unchanged by this increment** — 243/30/142 and 14/7 respectively (§4/§24.2/§24.3), carried forward unedited. Migration 221 (English passage-eligibility reconciliation) remains HOLD/NOT APPLIED, per its own governing architecture note (`ENGLISH_PASSAGE_ELIGIBILITY_ARCHITECTURE_NOTE.md`) requiring the reconciliation and its real code gate to be designed and proven together, not sequenced apart.

**What this does and does not change about §1's own verdict:** the family-depth exhaustion mathematics in §7 (51/73 families at only 2-4 variants) is essentially unaffected by a single new 8-row family — this is evidence of a real, working production/review/promotion pipeline now proven end-to-end for the first time this session, not evidence that the underlying capacity gap itself has materially closed. §24.10's "PARTIALLY READY" verdict stands unchanged by this section.
