...
const difficultyColor: Record<string, string> = {
  ...
  "year5-advanced": "bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300",
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 md:px-8 md:py-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="bg-yellow-100 dark:bg-yellow-900 p-3 rounded-2xl">
            <BookOpen size={22} aria-hidden="true" className="text-yellow-600 dark:text-yellow-400" />
          </div>
          <div>
        ...
        {/* Skills covered */}
        <div className="mt-4 bg-yellow-50 dark:bg-yellow-950 rounded-xl p-4">
          <p className="text-yellow-700 dark:text-yellow-300 text-xs font-semibold uppercase tracking-wide mb-2">
            Skills Covered
          </p>
          <div className="flex flex-wrap gap-2">
            {["Inference", "Evidence retrieval", "Vocabulary in context", "Atmosphere", "Character analysis", "Writer's techniques"].map((skill) => (
              <span key={skill} className="bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-300 text-xs font-medium px-2.5 py-1 rounded-full">
                {skill}
              </span>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-3">
          {filtered.map((lesson, i) => (
            <Link
              ...
              className="block bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800 hover:border-yellow-200 dark:hover:border-yellow-800 hover:shadow-sm active:scale-[0.98] transition-all motion-reduce:transition-none group"
            >
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-gray-900 dark:text-gray-100 font-semibold text-base mb-1 group-hover:text-yellow-700 dark:group-hover:text-yellow-300 transition-colors motion-reduce:transition-none">
                    {lesson.title}
                  </h3>
                ...
                <div className="flex items-center gap-2 ml-3">
                  {lesson.id === recommendedId ? (
                    <span className="bg-yellow-100 dark:bg-yellow-900 text-yellow-600 dark:text-yellow-400 text-xs font-semibold px-2 py-0.5 rounded-full">
                      Recommended
                    </span>
                  ) : !recommendedId && i === 0 ? (
                  ...
                  <ChevronRight size={18} aria-hidden="true" className="text-gray-300 dark:text-gray-600 group-hover:text-yellow-500 dark:group-hover:text-yellow-400 transition-colors motion-reduce:transition-none" />
                </div>
              </div>
            </Link>
          ))}
