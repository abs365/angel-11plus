 ...
 * Rewrites Increment 1's homepage content against the Founder's visual
 * review of the real production page: fewer repeated icon-in-circle cards,
 * an editorial layout instead of a SaaS feature grid, and the new
 * Angel Brand Foundation tokens (--angel-navy/-blue/-ivory/-paper/-gold/
 * -sky/-ink/-muted/-border, app/globals.css) in place of the generic
 * --text- and --surface- prefixed aliases Increment 1 used. Structure (routes,
 * metadata, access control) is unchanged from Increment 1 — this is a
 * content/visual pass only, per the governing instruction's scope
 * protection (§27).
 */
...
const JOURNEY = [
  { title: "Learn", body: "Understand something properly, with a clear example before trying it alone." },
  { title: "Practise", body: "Build confidence through varied questions, with support where it's needed." },
  { title: "Check", body: "See what your child can do independently, not just what they can copy." },
  { title: "Review", body: "Return to what needs strengthening, before it's forgotten." },
  { title: "Mock", body: "Measure independent performance under exam-style conditions." },
  { title: "Improve", body: "Use that evidence to decide honestly what comes next." },
];
      ...
      <main>
        ...
        <section className="max-w-7xl mx-auto px-4 md:px-8 pt-14 md:pt-24 pb-16 md:pb-28 grid md:grid-cols-12 gap-10 md:gap-16 items-center">
          <div className="md:col-span-7">
            <h1 className="text-4xl md:text-6xl font-bold leading-[1.05] tracking-tight text-[var(--angel-navy)] max-w-2xl">
              11+ preparation shaped around your child.
            </h1>
            <p className="mt-6 text-lg text-[var(--angel-muted)] leading-relaxed max-w-xl">
        ...
        <section id="how-it-works" className="scroll-mt-20 border-t border-[var(--angel-border)] bg-[var(--angel-ivory)]">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-24">
            <GoldRule />
            <h2 className="text-2xl md:text-3xl font-bold text-[var(--angel-navy)]">How Angel 11+ works</h2>
            <p className="mt-3 text-[var(--angel-muted)] max-w-xl leading-relaxed">
            ...
            <ol className="mt-12 grid gap-8 md:grid-cols-6 md:gap-6">
              {JOURNEY.map((step, i) => (
                <li
                  key={step.title}
                  className="pl-5 border-l-2 md:pl-0 md:border-l-0 md:border-t-2 border-[var(--angel-border)] md:pt-6"
                >
                  <span className="block text-xs font-bold tracking-widest text-[var(--angel-blue)]">
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <h3 className="mt-2 font-bold text-[var(--angel-navy)]">{step.title}</h3>
                  <p className="mt-1.5 text-sm text-[var(--angel-muted)] leading-relaxed">{step.body}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-24">
          <div className="grid md:grid-cols-12 gap-10 md:gap-16 items-start">
            <div className="md:col-span-5">
              ...
              <h2 className="mt-2 text-2xl md:text-3xl font-bold text-[var(--angel-navy)]">A clear plan for today</h2>
              <p className="mt-4 text-[var(--angel-ink)] leading-relaxed max-w-md">
            ...
            <div className="md:col-span-7 rounded-2xl bg-[var(--angel-sky)] p-6 md:p-10">
              <ol className="divide-y divide-[var(--angel-border)]">
                {TODAYS_PLAN.map((item) => (
                  <li key={item.title} className="py-4 first:pt-0 last:pb-0 flex items-baseline justify-between gap-6">
                    <div>
                      <p className="font-semibold text-[var(--angel-navy)]">{item.title}</p>
                      <p className="text-sm text-[var(--angel-muted)]">{item.detail}</p>
                    </div>
                    <span className="text-sm text-[var(--angel-muted)] whitespace-nowrap">{item.time}</span>
                  </li>
                ))}
              </ol>
              <p className="mt-6 text-sm text-[var(--angel-muted)] leading-relaxed">
          ...
          <div className="mt-20 md:mt-28 max-w-2xl">
            <h2 className="text-2xl md:text-3xl font-bold text-[var(--angel-navy)]">
              Preparation that responds to your child
            </h2>
            <p className="mt-4 text-[var(--angel-ink)] leading-relaxed">
            ...
            <p className="mt-6 text-lg font-bold text-[var(--angel-navy)]">Angel recommends. Family chooses.</p>
            <p className="mt-2 text-sm text-[var(--angel-muted)] leading-relaxed">
        ...
        <section id="what-you-practise" className="scroll-mt-20 border-t border-[var(--angel-border)] bg-[var(--angel-ivory)]">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-24">
            <h2 className="text-2xl md:text-3xl font-bold text-[var(--angel-navy)]">What children practise</h2>
            <p className="mt-3 text-[var(--angel-muted)] max-w-xl leading-relaxed">
            ...
            <dl className="mt-10 divide-y divide-[var(--angel-border)] max-w-3xl">
              {SUBJECTS.map((s) => (
                <div key={s.title} className="py-6 grid md:grid-cols-[10rem_1fr] gap-1 md:gap-10">
                  <dt className="font-bold text-[var(--angel-navy)]">{s.title}</dt>
                  <dd className="text-[var(--angel-ink)] leading-relaxed">{s.body}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <section id="for-parents" className="scroll-mt-20">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-24">
            <h2 className="text-2xl md:text-3xl font-bold text-[var(--angel-navy)]">For parents</h2>
            <p className="mt-3 text-[var(--angel-muted)] max-w-xl leading-relaxed">
            ...
            <div className="mt-12 grid md:grid-cols-3 gap-10 md:gap-8">
              <div>
                ...
                <ul className="mt-3 space-y-1.5 text-[var(--angel-ink)]">
                  {GOING_WELL.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              ...
              <div>
                ...
                <p className="mt-3 text-[var(--angel-ink)] leading-relaxed">
                  Focused inference practice, followed by a mixed comprehension check.
                </p>
              </div>
            </div>
            <p className="mt-4 text-xs text-[var(--angel-muted)]">
              Illustrative example. Not a real child&rsquo;s data.
            </p>

            <ul className="mt-12 max-w-xl space-y-3 border-t border-[var(--angel-border)] pt-10">
              {FOR_PARENTS.map((item) => (
                <li key={item} className="flex items-start gap-3">
                  <CheckCircle2 size={18} className="text-[var(--angel-blue)] mt-0.5 shrink-0" aria-hidden="true" />
                  <span className="text-[var(--angel-ink)] leading-relaxed">{item}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <section id="mock-tests" className="scroll-mt-20 border-t border-[var(--angel-border)] bg-[var(--angel-ivory)]">
          <div className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-24">
            ...
            <div className="mt-8 max-w-2xl">
              <p className="text-[var(--angel-ink)] leading-relaxed">
        ...
        {/* Our approach */}
        <section className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-24">
          <h2 className="text-2xl md:text-3xl font-bold text-[var(--angel-navy)]">Our approach</h2>
          <dl className="mt-10 grid sm:grid-cols-2 gap-x-16 gap-y-10 max-w-3xl">
            {APPROACH.map((p) => (
              <div key={p.title}>
                <dt className="font-bold text-[var(--angel-navy)]">{p.title}</dt>
                <dd className="mt-1.5 text-sm text-[var(--angel-ink)] leading-relaxed">{p.body}</dd>
              </div>
            ))}
          </dl>
        </section>
