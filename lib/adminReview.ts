 ...
 * 'mock_maths_independent_review'` and the generalised
 * deriveBatchReviewStatus()/fetchBatchReviewStatus() helpers unchanged.
 * REMEDIATED per Decision 200/201: the originally-authored 4th subpart
 * (a quadratic-equation reverse-reasoning demand) was found to have no
 * primary-source support and was removed; this family is now 3 rows/3
 * marks, not 4/4.
 *
 * mock-mr06-numberpuzzle is grouped: 1 numbered-question experience of 3
 * subparts, sharing one `prompt.sharedStem` across all 3 rows — TEXT-ONLY
...
export const MOCK_STRUCTURAL_CAPACITY_INCREMENT005_FAMILIES: SevenXFamilyConfig[] = [
  {
    ...
    disclosure:
      "This is a brand-new family with no prior review of any kind. It is a genuine interdependent algebraic-system compound, but a MATERIALLY DIFFERENT reasoning graph from mock-mr06-linkedvalues: no numeric total is ever stated, and a hidden positive whole number n is defined only via three derived rules (P = n+9, Q = 9xn, R = nxn) shared once via sharedStem. Subparts (a)-(c) are symbolic substitution-and-simplification exercises in which n algebraically cancels out, true for every positive n (independently re-verified this session by substituting two different concrete values of n and confirming the same constants both times). Directly evidenced this session against the real 2023 Q18 (independently re-verified against both the real paper and its mark scheme: A=B+3, C=3B, D=B^2, with confirmed answers 9/3/0 for 3A-C / C/(A-3) / AB-C-D), which mock-mr06-linkedvalues (built on the different 2021 Q20 total-constraint shape) does not itself represent. QT-MR-06 (Multiple-Relation/algebraic reasoning) is reused, not a new Question Type. Difficulty: (a) medium (linear-combination substitution), (b) medium (a division-based substitution, a materially different operation from (a)), (c) hard (expanding a product of two expressions containing a squared term, the deepest simplification of the three subparts). Disclosed honestly: this family does not add a further easy-tier row (mock-mr04-campingsale, migration 134, already did that); every label reflects genuine reasoning burden, not estate-statistic padding. No new visual capability required: text-only abstract-algebra content, no stimulus table. REMEDIATION DISCLOSURE (Decision 200/201): this family originally had a 4th subpart requiring a quadratic-equation reverse-reasoning demand (form and solve n^2-9n-70=0, reject a negative root). An educational evidence audit independently re-opened the real 2023 Q18 source and found no support for that demand anywhere in it, and checked the other 4 evidenced occurrences of this archetype (2023 Q8, 2022 Q6, 2021 Q7, 2021 Q20) and found none of them ever exceeds 3 subparts or requires reverse/non-linear reasoning either. The 4th subpart was removed before this family was ever submitted for review; this is now, and has only ever been reviewable as, a 3-row/3-mark family. Disclosed for your own judgement, not a recommendation either way.",
  },
];
