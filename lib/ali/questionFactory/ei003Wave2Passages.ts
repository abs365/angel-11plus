/**
 * Educational Increment 003, Wave 2 — six new, original passages for the
 * five thin English inference/interpretation families
 * (wave3-fam-rc06-sequencing, rc07-comparative, rc08-emotion,
 * rc10-atmosphere-mood, rc10-word-choice).
 *
 * All original Angel content -- no commercial text reproduced, no
 * identifiable copyrighted passage imitated. Deliberately longer and
 * richer than the existing wave3-eng-* passages (695-855 characters,
 * confirmed via live production read this wave) -- these run
 * ~1,800-2,300 characters, matching the bank's own established
 * Increment 002 convention, because sequencing/comparative/emotion-arc
 * reasoning genuinely needs more narrative development than a single
 * atmospheric moment does. Each passage is written to genuinely support
 * MULTIPLE of the five target families (chronology for sequencing,
 * character/reaction pairs for comparative, indirect behavioural cues
 * for emotion inference, sensory/atmospheric language for mood, precise
 * vivid word choices for word-choice) -- never a single-purpose
 * container built around one obvious answer.
 *
 * Genre coverage, deliberately targeting the plan's own named gaps
 * (ANGEL_EDUCATIONAL_INCREMENT_003_SUSTAINED_LEARNING_PLAN.md §M):
 * biography/human-interest, travel/exploration, and descriptive prose
 * were explicitly missing from the existing bank; added here alongside
 * two further fiction/narrative passages (needed for de-clustering --
 * a single narrative passage cannot alone carry sequencing AND
 * comparative AND emotion-arc questions without over-concentration) and
 * one nature/science informational piece.
 */

export type PassageGenre = "fiction_narrative" | "nature_science" | "human_interest_memoir" | "travel_exploration" | "descriptive_prose";

export interface Wave2Passage {
  id: string;
  title: string;
  genre: PassageGenre;
  text: string;
...
export const EI003_WAVE2_PASSAGES: Wave2Passage[] = [
  {
    id: "ei003-w2-eng-the-relay-baton",
    title: "The Relay Baton",
    genre: "fiction_narrative",
    text: `Nadia had run the second leg of every relay since Year 4, and every time, the wait was the worst part. She stood at the changeover line watching Priya sprint towards her, the baton a small orange blur in her fist, and Nadia's own legs felt suddenly unfamiliar, as though she'd forgotten how they worked.
...
It was only that evening, unlacing her trainers at home, that Nadia realised she still had a faint red mark across two knuckles where the baton had struck them.`,
  },
  {
    id: "ei003-w2-eng-the-last-delivery",
    title: "The Last Delivery",
    genre: "fiction_narrative",
    text: `Grandad Kofi had driven the same milk round for thirty-one years, and Tobias had begged for months to come along on the very last one. Now, sitting up in the passenger seat of the small electric float, he wasn't sure what he'd expected — something more like a celebration, maybe, with people waving from doorsteps.
...
When he got back into the float, he didn't start the engine straight away. He sat with both hands resting on the wheel, looking out at the empty street ahead, and Tobias, watching him, decided not to ask anything else.`,
  },
  {
    id: "ei003-w2-eng-the-glass-frog",
    title: "The Glass Frog's Hidden Trick",
    genre: "nature_science",
    text: `Most animals that want to avoid being eaten rely on hiding, running, or looking dangerous. The glass frog, a small amphibian found in the rainforests of Central and South America, does something stranger: while it sleeps during the day, clinging to the underside of a leaf, much of its body becomes almost see-through.
...
For now, the discovery has mostly reshaped how scientists think about camouflage itself. It had long been assumed that transparency in animals worked primarily through skin structure — thin, light-scattering tissue that bends light rather than reflecting it. The glass frog shows that an animal can achieve much the same effect through an entirely different route: not by changing what its skin is made of, but by managing, with remarkable precision, what is allowed to flow beneath it.`,
  ...
  {
    id: "ei003-w2-eng-the-clock-that-stopped",
    title: "The Clock That Stopped",
    genre: "human_interest_memoir",
    text: `I was eleven when my grandmother taught me to mend a clock, and I still remember exactly which one: a heavy mantel clock with a cracked face that had stopped, according to family legend, on the afternoon my great-grandfather left for the war and never been persuaded to run properly since.
...
I think about that clock now whenever something in my own life refuses to work the way I expect, and someone tells me it's simply broken, always was, always will be. I think about the gear installed backwards for who knows how many decades, silently wrong, waiting for someone with enough patience to notice.`,
  },
  {
    id: "ei003-w2-eng-crossing-the-fen",
    title: "Crossing the Fen",
    genre: "travel_exploration",
    text: `The guide, a weathered man named Onyema who had walked the causeway more times than he could count, warned us before we set out that the fen changed with every high tide, and that a path safe at dawn could be treacherous by noon.
...
Nothing in the room had been designed to be seen this way, gathered and left, and yet the effect — accidental, unplanned — was somehow more complete than any deliberately arranged display could have managed: an entire vanished routine, preserved exactly at the moment it stopped.`,
  },
];

export function getWave2PassageById(id: string): Wave2Passage {
  const passage = EI003_WAVE2_PASSAGES.find((p) => p.id === id);
  if (!passage) throw new Error(`getWave2PassageById: no passage found for id "${id}"`);
  return passage;
}
