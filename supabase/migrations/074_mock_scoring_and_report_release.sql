...
-- migration 072's own omission (Decisions 94/95) established.
--
...
--
-- APPLIED to production (confirmed by the Founder). This file's own SQL
-- below is preserved exactly as it was actually applied — this is the
-- true migration-history record, per this project's own established
-- "074 represents what was actually applied" principle (Decision 99).
--
-- STATUS CORRECTION: a subsequent Founder pre-application architecture
-- review (conducted under the mistaken belief that this migration had
-- not yet been applied — corrected in Decision 99) identified three
-- genuine design issues with the functions this file creates: mock_
-- score_attempt was directly executable by an ordinary authenticated
-- learner (no technical reason for this — its only real caller was
-- browser client code); scoring_state could read 'scored' while
-- questions still required manual marking (a real premature-release
-- risk mock_release_report's own gate could not see through); and a
-- response row with a null/empty value could be mis-scored 'incorrect'
-- rather than 'unanswered'. All three are real, and all three are
-- corrected by supabase/migrations/075_mock_scoring_trust_boundary_
-- correction.sql — a forward-only migration, NOT a rewrite of this one,
-- since this one was already applied. See Decisions 98 (the original,
-- factually-mistaken pre-application review) and 99 (the correction and
-- 075 itself) in ALI_DECISION_LOG.md for the full history. This file's
-- own original text above (this comment block, not its executable SQL)
-- has been lightly corrected in place to remove a now-false claim that a
-- follow-up hardening migration should not be needed — the executable
-- SQL below is untouched from what was actually applied.
